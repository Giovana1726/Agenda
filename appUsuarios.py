...
class Application:
    ...
    def exportar_para_pdf(self, Usuario=None):
        # Instanciar a classe Usuarios para obter os dados
        usuario_obj = Usuario()
        usuario = usuario_obj.selectAllUsuarios()

        if isinstance(usuario, str):

            messagebox.showerror("Erro", usuario)
            return
        ...
        try:
            with PdfPages(pdf_file) as pdf:
                ...
                tbl = ax.table(
                    cellText=usuario,
                    colLabels=["ID", "Nome", "Telefone", "E-mail", "Usuário"],
                    cellLoc='center',
                    loc='center',
                    bbox=[0, 0, 1, 1]  # Ajuste a tabela para preencher a figura
                )
